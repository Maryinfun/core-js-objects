...
function shallowCopy(obj) {
  const spreadedObj = { ...obj };
  return Object.assign(spreadedObj);
}
...
function makeWord(lettersObject) {
  const lettersObjectArr = Object.entries(lettersObject);
  const word = [];
  lettersObjectArr.forEach(([key, value]) => {
    if (value.length > 1) {
      value.forEach((el) => {
        word[el] = key;
      });
...
function sellTickets(queue) {
  const count25 = queue.filter((el) => el === 25).length;
  const count50 = queue.filter((el) => el === 50).length;
  const count100 = queue.filter((el) => el === 100).length;
  return count25 >= count50 + 3 * count100;
}
